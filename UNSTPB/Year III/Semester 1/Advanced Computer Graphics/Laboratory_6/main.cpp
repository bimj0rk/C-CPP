...
glm::vec3 cameraPosition = glm::vec3(0.0f, 30.0f, 100.0f);
glm::vec3 cameraViewDirection = glm::vec3(0.0f, 0.0f, -1.0f);
glm::vec3 cameraUp = glm::vec3(0.0f, 1.0f, 0.0f);
Camera camera(cameraPosition, cameraViewDirection, cameraUp);
...
void processInput(GLFWwindow* window) {
	...
	if (glfwGetKey(window, GLFW_KEY_W) == GLFW_PRESS) camera.translateFront(cameraSpeed);
	if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS) camera.translateBack(cameraSpeed);
	if (glfwGetKey(window, GLFW_KEY_R) == GLFW_PRESS) camera.translateUp(cameraSpeed);
	if (glfwGetKey(window, GLFW_KEY_F) == GLFW_PRESS) camera.translateDown(cameraSpeed);
}
...
int main(void)
{
	...
	glm::mat4 view;
	view = glm::lookAt(camera.getCameraPosition(), camera.getCameraViewDirection(), camera.getCameraUp());
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		view = glm::lookAt(glm::vec3(camX, 30.0f, camZ), camera.getCameraViewDirection(), camera.getCameraUp());
		*/
		// Swap buffers
