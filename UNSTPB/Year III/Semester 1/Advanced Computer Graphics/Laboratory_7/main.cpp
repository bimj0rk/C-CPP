...
glm::vec3 cameraPos = glm::vec3(0.0f, 0.0f, 350.0f);
...
float ka = 0.3;
glm::vec3 ambient = ka * lightColor;
...
int main(void)
{
	...
	while (!glfwWindowShouldClose(window) && glfwGetKey(window,GLFW_KEY_ESCAPE)!=GLFW_PRESS)
	{
		...
		//light
		unsigned int lightLoc = glGetUniformLocation(programID, "lightPos");
		glUniform3fv(lightLoc, 1, glm::value_ptr(ambient));
		...
		glUniform4fv(transformLoc2, 1, glm::value_ptr(result));
