...
int main(void)
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (int i = 0; i < 10; i++)
		{
			...
			glm::mat4 view = glm::lookAt(glm::vec3(0.0f, 15.0f, 90.0f), glm::vec3(0.0f, 0.0f, -1.0f), glm::vec3(0.0f, 0.5f, 0.0f));
			glm::mat4 projection = glm::perspective(glm::radians(60.0f), (float)width / (float)height, 0.1f, 10000.f);
			glm::mat4 mvpMat = projection * view * model;

			unsigned int transformLoc = glGetUniformLocation(programID, "transform");
			glUniformMatrix4fv(transformLoc, 1, GL_FALSE, glm::value_ptr(mvpMat));

			unsigned int transformLoc2 = glGetUniformLocation(programID, "color");
			glUniform4fv(transformLoc2, 1, glm::value_ptr(glm::vec4(0.5f + i / 10.0f, 1 - i / 10.f, 0.5f, 1.0)));
